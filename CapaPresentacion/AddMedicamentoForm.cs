    ...
    public partial class AddMedicamentoForm : Form
    {
        ...
        private void dgMedicamentos_SelectionChanged(object sender, EventArgs e)
        {
            ...
            if (hayFilaSeleccionada)
            {
                ...
                var proveedorId = fila.Cells["id_proveedor"].Value != DBNull.Value ? Convert.ToInt32(fila.Cells["id_proveedor"].Value) : 0;
                // Verificar si el id_proveedor es válido
                if (proveedorId == 0)
                {
                    // Seleccionar "Sin proveedor" en el ComboBox si no hay proveedor
                    cbProveedores.SelectedValue = 0;
                }
                else
                {
                    // Si el proveedor existe, asignamos el id al ComboBox
